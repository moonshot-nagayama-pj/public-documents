...
The range of time scales of interest range from ensuring the sub-wavelength stability of optical paths up to batch monitoring of the operation of the network itself. Light with a wavelength of $$1.5\u\text{m}$$ (common in communications, including quantum communications) has a frequency of approximately $$200$$ THz ($$2\times 10^{14}$$ Herz).  Ranging from sub-wavelength stabilization through batch operations, therefore, covers some 16 or more decimal orders of magnitude.  Naturally, meeting this range of demands a variety of mechanisms.  This document avoids specifying solutions to the problems, and instead presents the functions and how their requirements are calculated (or measured).  Thus, each individual network design should apply the methods introduced here and present a numerical summary of the resulting values, after which corresponding solutions can be proposed and implemented.
...
* __Near-real time computation__: many quantum algorithms are hybrid classical/quantum computation and larger-scale adaptation or recompilation of application quantum circuits.
